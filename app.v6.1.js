...
function afficherHistoiresSauvegardees() {
  ...
  histoires.forEach((histoire, index) => {
    const li = document.createElement("li");
    li.innerHTML = `
      <button class="button btn-histoire" data-index="${index}">${histoire.titre}</button>
...
// Gérer le clic sur un bouton d’histoire uniquement si la sélection n’est pas active
document.getElementById("liste-histoires").addEventListener("click", (e) => {
  if (e.target.classList.contains("btn-histoire")) {
    if (modeSelectionActive) {
      console.log("🛑 Clic ignoré car mode sélection actif");
      return;
    }
    const index = parseInt(e.target.getAttribute("data-index"));
    console.log("➡️ Navigation vers l’histoire", index);
