 ...
 */
MonHistoire.modules.sharing.realtime.listeners = {
  ...
  init() {
    ...
    if (!MonHistoire.modules.sharing.histoiresPartageesListener) {
      MonHistoire.modules.sharing.histoiresPartageesListener = [];
    }
    ...
    try {
      ...
      if (!user) {
        ...
        const authListener = firebase.auth().onAuthStateChanged(newUser => {
          if (newUser && !MonHistoire.modules.sharing.histoiresPartageesListener.length) {
      ...
      if (!MonHistoire.state.isConnected) {
        ...
        const connectionListener = (isConnected) => {
          if (isConnected && !MonHistoire.modules.sharing.histoiresPartageesListener.length) {
            // Détacher cet écouteur pour éviter les doublons
      ...
      if (MonHistoire.state.realtimeDbAvailable === false) {
        ...
        const dbListener = (isAvailable) => {
          if (isAvailable && !MonHistoire.modules.sharing.histoiresPartageesListener.length) {
            // Détacher cet écouteur pour éviter les doublons
  ...
  planifierNouvelleTentative() {
    ...
    setTimeout(() => {
      if (!MonHistoire.modules.sharing.histoiresPartageesListener || 
          !MonHistoire.modules.sharing.histoiresPartageesListener.length) {
  ...
  detacherEcouteursHistoiresPartagees() {
    if (MonHistoire.modules.sharing.histoiresPartageesListener) {
      if (Array.isArray(MonHistoire.modules.sharing.histoiresPartageesListener)) {
        MonHistoire.modules.sharing.histoiresPartageesListener.forEach(listener => {
      ...
      } else if (typeof MonHistoire.modules.sharing.histoiresPartageesListener === 'function') {
        try {
          MonHistoire.modules.sharing.histoiresPartageesListener();
        } catch (e) {
  ...
  configurerEcouteurProfilActif(user) {
    ...
    const listener = storiesRef.onSnapshot(snapshot => {
      ...
      snapshot.docChanges().forEach(change => {
        ...
        if (change.type === "added" || change.type === "modified") {
          ...
          if (data.partageParPrenom && !estPartageParProfilActif && !MonHistoire.modules.sharing.notificationsTraitees.has(notificationId)) {
            ...
            if (MonHistoire.modules.sharing.notifications) {
              MonHistoire.modules.sharing.notifications.afficherNotificationPartage(data.partageParPrenom, change.doc.ref);
            }
          }
        }
      });
    ...
    MonHistoire.modules.sharing.histoiresPartageesListener.push(listener);
  },
          ...
          const listener = storiesRef.onSnapshot(storiesSnapshot => {
            ...
            storiesSnapshot.docChanges().forEach(change => {
              if (change.type === "added" || change.type === "modified") {
                ...
                if (data.partageParPrenom && !MonHistoire.modules.sharing.notificationsTraitees.has(notificationId)) {
                  // Ajouter l'ID à la liste des notifications traitées
                  MonHistoire.modules.sharing.notificationsTraitees.add(notificationId);
                  ...
                  if (MonHistoire.modules.sharing.notifications) {
                    MonHistoire.modules.sharing.notifications.afficherNotificationPartage(message, change.doc.ref);
                  }
                }
              }
            });
