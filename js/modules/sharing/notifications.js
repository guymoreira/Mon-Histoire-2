...
MonHistoire.modules.sharing.notificationsNonLues = MonHistoire.modules.sharing.notificationsNonLues || {};
MonHistoire.modules.sharing.notificationsTraitees = MonHistoire.modules.sharing.notificationsTraitees || new Set();
MonHistoire.modules.sharing.notificationTimeout = null;
MonHistoire.modules.sharing.notificationSwipeStartX = 0;
MonHistoire.modules.sharing.notificationSwipeStartY = 0;
...
MonHistoire.modules.sharing.notifications = {
  ...
  init() {
    ...
    try {
      ...
      if (cachedNotifications) {
        MonHistoire.modules.sharing.notificationsTraitees = new Set(JSON.parse(cachedNotifications));
      }
    } catch (e) {
    ...
    if (MonHistoire.modules.sharing) {
      MonHistoire.modules.sharing.mettreAJourIndicateurNotification = this.mettreAJourIndicateurNotification.bind(this);
      MonHistoire.modules.sharing.mettreAJourIndicateurNotificationProfilsListe = this.mettreAJourIndicateurNotificationProfilsListe.bind(this);
    }
  },
  ...
  async initialiserCompteurNotifications(user) {
    try {
      ...
      MonHistoire.modules.sharing.notificationsNonLues[profilId] = snapshot.size;
      ...
      if (MonHistoire.state.profilActif.type === "parent") {
        ...
        for (const profilDoc of profilsEnfantsSnapshot.docs) {
          ...
          MonHistoire.modules.sharing.notificationsNonLues[enfantProfilId] = 0;
          ...
          MonHistoire.modules.sharing.notificationsNonLues[enfantProfilId] = storiesEnfantSnapshot.size;
        }
      }
      
      if (MonHistoire.logger) {
        MonHistoire.logger.sharingInfo("Compteur de notifications initialisé", {
          notificationsCount: Object.keys(MonHistoire.modules.sharing.notificationsNonLues).length,
          status: "OK"
        });
      } else {
        console.log("Compteur de notifications initialisé:", MonHistoire.modules.sharing.notificationsNonLues);
      }
    } catch (error) {
      if (MonHistoire.logger) {
  ...
  afficherNotificationPartage(prenomPartageur, histoireRef) {
    ...
    MonHistoire.modules.sharing.histoireNotifieeActuelle = histoireRef;
    ...
    if (MonHistoire.modules.sharing.notificationTimeout) {
      clearTimeout(MonHistoire.modules.sharing.notificationTimeout);
      MonHistoire.modules.sharing.notificationTimeout = null;
    }
    ...
    MonHistoire.modules.sharing.notificationTimeout = setTimeout(() => {
      this.fermerNotificationPartage();
    }, 5000);
  },
  ...
  fermerNotificationPartage(marquerCommeVue = true) {
    ...
    if (marquerCommeVue && MonHistoire.modules.sharing.histoireNotifieeActuelle) {
      // Récupère l'ID du profil actif
      const profilId = MonHistoire.state.profilActif.type === "parent" ? "parent" : MonHistoire.state.profilActif.id;
      
      // Décrémente le compteur de notifications non lues
      if (MonHistoire.modules.sharing.notificationsNonLues[profilId] && MonHistoire.modules.sharing.notificationsNonLues[profilId] > 0) {
        MonHistoire.modules.sharing.notificationsNonLues[profilId]--;
      }
      
      try {
        // Vérifier que histoireNotifieeActuelle est valide avant d'appeler get()
        if (MonHistoire.modules.sharing.histoireNotifieeActuelle && 
            typeof MonHistoire.modules.sharing.histoireNotifieeActuelle.get === 'function') {
          
          // Met à jour le document Firestore
          MonHistoire.modules.sharing.histoireNotifieeActuelle.get().then(doc => {
            if (doc.exists) {
              const data = doc.data();
              ...
              if (MonHistoire.modules.sharing.histoireNotifieeActuelle && 
                  typeof MonHistoire.modules.sharing.histoireNotifieeActuelle.update === 'function') {
                
                // Met à jour le document
                MonHistoire.modules.sharing.histoireNotifieeActuelle.update({ 
      ...
      MonHistoire.modules.sharing.histoireNotifieeActuelle = null;
  ...
  demarrerSwipeNotification(e) {
    ...
    if (e.type === "touchstart") {
      MonHistoire.modules.sharing.notificationSwipeStartX = e.touches[0].clientX;
      MonHistoire.modules.sharing.notificationSwipeStartY = e.touches[0].clientY;
    } else {
      MonHistoire.modules.sharing.notificationSwipeStartX = e.clientX;
      MonHistoire.modules.sharing.notificationSwipeStartY = e.clientY;
    }
  },
  ...
  deplacerSwipeNotification(e) {
    ...
    if (MonHistoire.modules.sharing.notificationSwipeStartX === 0 && MonHistoire.modules.sharing.notificationSwipeStartY === 0) return;
    ...
    const distanceX = Math.abs(currentX - MonHistoire.modules.sharing.notificationSwipeStartX);
    const distanceY = Math.abs(currentY - MonHistoire.modules.sharing.notificationSwipeStartY);
  ...
  mettreAJourIndicateurNotification() {
    try {
      ...
      if (!MonHistoire.modules.sharing.notificationsNonLues) {
        MonHistoire.modules.sharing.notificationsNonLues = {};
      }
      
      // Récupère le nombre de notifications non lues pour ce profil
      const nbNotifs = MonHistoire.modules.sharing.notificationsNonLues[profilId] || 0;
    ...
    try {
      ...
      if (MonHistoire.logger) {
        MonHistoire.logger.sharingInfo("Notifications recalculées pour le profil", {
          profilId: profilId,
          count: MonHistoire.modules.sharing.notificationsNonLues[profilId]
        });
      }
      ...
      return MonHistoire.modules.sharing.notificationsNonLues[profilId];
    } catch (error) {
      if (MonHistoire.logger) {
  ...
  mettreAJourIndicateurNotificationProfilsListe() {
    try {
      ...
      profilItems.forEach(item => {
        ...
        if (profilId && MonHistoire.modules.sharing.notificationsNonLues[profilId] && MonHistoire.modules.sharing.notificationsNonLues[profilId] > 0) {
          ...
          // Met à jour le contenu de l'indicateur
          indicateur.textContent = MonHistoire.modules.sharing.notificationsNonLues[profilId] > 9 ? "9+" : MonHistoire.modules.sharing.notificationsNonLues[profilId].toString();
          indicateur.style.display = "flex";
        } else {
