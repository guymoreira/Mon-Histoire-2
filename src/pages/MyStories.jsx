    ...
    <div className="flex justify-center">
      <Card>
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-2xl font-bold">Mes Histoires</h2>
          <span id="compteur-histoires" className={`text-xl ${stories.length >= 8 ? 'text-red-600 animate-pulse' : 'text-primary'}`}>
            {stories.length} / 10
          </span>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-4 text-gray-500 italic">Aucune histoire sauvegardée.</div>
        ) : (
          <ul id="liste-histoires" className="mb-6">
            {stories.map(story => (
              <li key={story.id} className={story.nouvelleHistoire ? 'nouvelle-histoire' : ''}>
                <StoryCard 
                  story={story} 
